...
class ActivityPersisterDF(ActivityPersister):
    def __init__(self, activity_df:pd.DataFrame|None, *, max_activites_save:int=15) -> None:
        ...
        if activity_df is not None:
            ...
            self._activity_df = activity_df.copy(deep=True)

        self._activity_count = len(self._activity_df)

    @property
    def game_df(self)->pd.DataFrame:
        return self._activity_df.copy(deep=True)
    
    def save_to_disk(self, pickle_path:str):
        self._activity_df.to_pickle(pickle_path)
    ...
    def delete_game(self, activity_id:uuid.UUID):
        drop_index:pd.Index[Any] = self._activity_df[self._activity_df['activity_id'] == activity_id].index
        if len(drop_index) == 0:
            raise ValueError(f"Activity with {str(activity_id)} not found!")

        self._activity_df.drop(drop_index, inplace=True)
