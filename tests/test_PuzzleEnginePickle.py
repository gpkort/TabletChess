...
from GameManager import PuzzleEngineDF, Puzzle, Theme, DEFAULT_PERSIST_DF
...
def basic_puzzles():
    ...
    for i in range(10):
        ...
        game_df = pd.concat([puzzle_df, pd.DataFrame([asdict(puzzle)])])    
...
class TestPuzzleEngineDataFrame:
    ...
    def test_init_with_df(self, basic_puzzles):
        gp:PuzzleEngineDF = PuzzleEngineDF(basic_puzzles,DEFAULT_PERSIST_DF)
